...
describe('AppNavbar.vue', () => {
  ...
  // Re-enabled after the fee-interval/advance-payment-ceiling redesign (see
  // FeeCalculationInfoBox.vue) resolved the overpayment/donation vs.
  // advance-payment ambiguity this entry was hidden for.
  it('shows "Mein Beitragskonto" for fee members', async () => {
  ...
  it('hides "Mein Beitragskonto" for non-fee members', async () => {
  ...
  it('navigates to the fee-account page and closes the drawer', async () => {
    mockAuthStore.user = { cn: 'Maria Muster', default_image: null, is_fee_member: true }
    const wrapper = mount(AppNavbar, {
      global: { plugins: [PrimeVue] },
      attachTo: document.body,
